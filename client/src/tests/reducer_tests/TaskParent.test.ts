...
import { AddTaskParent, DeleteTaskParent, UpdateTaskParent } from "../../redux/modules/TaskData/TaskDataActions";
...
describe('TaskDataReducers with TaskParent', () => {
    ...
    it('should return the initial state', () => {
        expect(TaskDataReducers(undefined, { type: "TEST_ACTION" })).toEqual(initialTaskDataState);
    });
    ...
    it('should handle ' + AddTaskParent.name + ' action', () => {
        ...
        expectedState.workspaces.currentWorkspace.setTaskParentIds(currentWorkpace.getTaskParentIds().concat(taskParent1.getId()));
        expectedState.workspaces.byId[currentWorkpace.getId()].setTaskParentIds(currentWorkpace.getTaskParentIds().concat(taskParent1.getId()));
        ...
        expectedState.taskParents.allIds = expectedState.taskParents.allIds.concat([taskParent1.getId()]);
    ...
    it("should handle " + DeleteTaskParent.name + " action", () => {
        ...
        let stateWithOneTp = TaskDataReducers(dummyInitialState, new AddTaskParent(taskParent1));
        let stateWithTwoTp = TaskDataReducers(stateWithOneTp, new AddTaskParent(taskParent2));

        // Delete just taskparent 2
        let stateAfterDeletion = TaskDataReducers(stateWithTwoTp, new DeleteTaskParent(taskParent2.getId()));
